...
#define _SYSCALL_LCD_INVERSE_PIXEL		(_SYSCALL_LCD_PRINTF+1)
...
void mBT_FlushBuffer(void)
{
	int RetVal;
	
	SysCallLib(_SYSCALL_BT_FLUSH_BUFFER, &RetVal, 0, 0);
}
...
void mBarcodeForm_FormatOperation(char *BarcodeStr)
{
	...
	SysCallLib(_SYSCALL_SCAN_BARCODE_FORMAT, &RetVal, Argc, Arg);
}
...
void mUSART1_Configure(int BaudRate)
{
	...
	Arg[0] = BaudRate;
	SysCallLib(_SYSCALL_USART_CONFIGURE, &RetVal, Argc, Arg);	
}
...
void mUSART1_SendDataByte(char data)
{
	...
	Arg[0] = data;
	SysCallLib(_SYSCALL_USART_SEND_DATABYTE, &RetVal, Argc, Arg);	
}
...
void mUSART1_SendDataStr(char *dataStr, int len)
{
	...
	Arg[0] = (unsigned int)dataStr;
	Arg[1] = len;
	SysCallLib(_SYSCALL_USART_SEND_STRING, &RetVal, Argc, Arg);
}
...
int mUSART1_ReceiveData(char *data)
{
	...
	Arg[0] = (unsigned int)data;
	SysCallLib(_SYSCALL_USART_RECEIVE, &RetVal, Argc, Arg);
